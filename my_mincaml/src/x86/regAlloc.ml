  ...
  | Let((x, t) as xt, exp, e) ->
      ...
      | Spill(y) ->
          ...
          let save =
            ...
            with Not_found -> Nop in (*変数yはまだ使っていないので退避不要*)           
          (seq(save, concat e1' (r, t) e2', Mloc.d_asm_seq), regenv2)
      | Alloc(r) ->
          let (e2', regenv2) = g dest cont (add x r regenv1) e in
          (concat e1' (r, t) e2', regenv2))
) in ((fun ((ta,_),regenv) mloc -> ((ta,mloc),regenv)) first (snd e)) (*この変更何? -> あぁ, ここで関数適用しているだけか*)
and g'_and_restore dest cont regenv exp mloc = (* 使用される変数をスタックからレジスタへRestore (caml2html: regalloc_unspill) *)
  ...
  with NoReg(x, t) ->
    ((* Format.eprintf "restoring %s@." x; *)
     g dest cont regenv ((Let((x, t), Restore(x), (Ans(exp),mloc))),mloc)) (*ここに恐らく位置情報を伝播させないとダメ*)
...
and g'_if dest cont regenv exp constr e1 e2 = (* ifのレジスタ割り当て (caml2html: regalloc_if) *)
  ...
  (fst(List.fold_left
     (fun e x ->
       ...
       seq(Save(M.find x regenv, x), e, Mloc.d_asm_seq)) (* そうでない変数は分岐直前にセーブ *) (*ここのmloc?*)
     (Ans(constr e1' e2'),Mloc.d_regalloc_g'if) (*ダミー化?*)
     (fv cont)),
   regenv')
and g'_call dest cont regenv exp constr ys zs = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  (fst(List.fold_left
     (fun e x ->
       ...
       seq(Save(M.find x regenv, x), e, Mloc.d_asm_seq)) (*ここのmloc?*)
     (Ans(constr
            (List.map (fun y -> find y Type.Int regenv) ys)
            (List.map (fun z -> find z Type.Float regenv) zs)),Mloc.d_regalloc_g'call) (*ダミー化*)
     (fv cont)),
   M.empty)
...
let f (Prog(data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  ...
  let e', _(*regenv'*) = g (Id.gentmp Type.Unit, Type.Unit) (Ans(Nop),Mloc.d_regalloc) M.empty e in (*ダミー化?*)
  Prog(data, fundefs', e')
