...
let rec target' src (dest, t) = function
  | Mov(x) when x = src && is_reg dest ->
      if not(t <> Type.Unit) then failwith "unit not expected";
      if not(t <> Type.Float) then failwith "float not expected";
      false, [dest]
  | FMov(x) when x = src && is_reg dest ->
      if not(t = Type.Float) then failwith "expected floats";
      false, [dest]
  ...
  | CallDir(_, ys, zs) ->
      true, (target_args src regs 0 ys @
             target_args src fregs 0 zs)
  | _ -> false, []
and target src dest e =  (* register targeting (caml2html: regalloc_target) *)
match (fst e) with  
...
let rec alloc dest cont regenv x t =
  ...
  (if (M.mem x regenv) then failwith "alloced before use");
  let all =
    match t with
    | Type.Unit -> ["%g0"] (* dummy *)
    | Type.Float -> allfregs
...
let add x r regenv =
  if is_reg x then (if not(x = r) then failwith "in add"; regenv) else
  M.add x r regenv
...
let rec g dest cont regenv e = (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
let first = 
(match (fst e) with
  | Ans(exp) -> g'_and_restore dest cont regenv exp (snd e)
  | Let((x, t) as xt, exp, e) ->
      (if (M.mem x regenv) then failwith "let alloced before use");
      let cont' = concat e dest cont in
      let (e1', regenv1) = g'_and_restore xt cont' regenv exp (snd e) in
      ...
      | Spill(y) ->
          ...
          (seq(save, concat e1' (r, t) e2', Mloc.d_asm_seq), regenv2)
      | Alloc(r) ->
          let (e2', regenv2) = g dest cont (add x r regenv1) e in
          (concat e1' (r, t) e2', regenv2))
) in ((fun ((ta,_),regenv) mloc -> ((ta,mloc),regenv)) first (snd e))
and g'_and_restore dest cont regenv exp mloc = (* 使用される変数をスタックからレジスタへRestore (caml2html: regalloc_unspill) *)
  try g' dest cont regenv exp mloc
  with NoReg(x, t) ->
    ((* Format.eprintf "restoring %s@." x; *)
    g dest cont regenv ((Let((x, t), Restore(x), (Ans(exp),mloc))),mloc))
and g' dest cont regenv exp mloc = (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
let without_Mloc = (
  match exp with
  ...
  | FLd(x, y') -> (Ans(FLd(find x Type.Int regenv, find' y' regenv)), regenv)
  | FSt(x, y, z') -> (Ans(FSt(find x Type.Float regenv, find y Type.Int regenv, find' z' regenv)), regenv)
  ...
  | Save(x, y) -> (*assert false*) failwith "save case not expected"
) in ((fst without_Mloc,mloc),snd without_Mloc)
and g'_if dest cont regenv exp constr e1 e2 = (* ifのレジスタ割り当て (caml2html: regalloc_if) *)
  ...
  (fst (List.fold_left
     (fun e x ->
       ...
       seq(Save(M.find x regenv, x), e, Mloc.d_asm_seq)) (* そうでない変数は分岐直前にセーブ *)
     (Ans(constr e1' e2'),Mloc.d_regalloc_g'if)
     (fv cont)),
   regenv')
and g'_call dest cont regenv exp constr ys zs = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  (fst (List.fold_left
     (fun e x ->
       ...
       seq(Save(M.find x regenv, x), e, Mloc.d_asm_seq))
     (Ans(constr
            (List.map (fun y -> find y Type.Int regenv) ys)
            (List.map (fun z -> find z Type.Float regenv) zs)),Mloc.d_regalloc_g'if)
     (fv cont)),
   M.empty)
  ...
  let (i, arg_regs, regenv) =
    List.fold_left
      (fun (i, arg_regs, regenv) y ->
        ...
        (i + 1,
         arg_regs @ [r],
         (if (is_reg y) then failwith "intreg not expected";
          M.add y r regenv)))
      (0, [], regenv)
      ys in
  let (d, farg_regs, regenv) =
    List.fold_left
      (fun (d, farg_regs, regenv) z ->
        ...
        (d + 1,
         farg_regs @ [fr],
         (if (is_reg z) then failwith "intreg not expected";
          M.add z fr regenv)))
      (0, [], regenv)
      zs in
  let a =
    ...
    | Type.Float -> fregs.(0)
    | _ -> regs.(0) in
  let (e', regenv') = g (a, t) (Ans(Mov(a)),Mloc.d_regalloc) regenv e in
  { name = Id.L(x); args = arg_regs; fargs = farg_regs; body = e'; ret = t }

let f (Prog(data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  Format.eprintf "register allocation: may take some time (up to a few minutes, depending on the size of functions)@.";
  let fundefs' = List.map h fundefs in
  let e', regenv' = g (Id.gentmp Type.Unit, Type.Unit) (Ans(Nop),Mloc.d_regalloc) M.empty e in
  Prog(data, fundefs', e')
