...
let alias_list = [("min_caml_fabs","min_caml_abs_float");("min_caml_print_char","min_caml_print_byte")]
...
match (fst e) with
  ...
  | Closure.Get(x, y) -> (* 配列の読み出し (caml2html: virtual_get) *)
      ...
      | Type.Array(Type.Float) -> (*Ans(FLd(x, V(y), 8)) SPARCの場合*)
          Let((offset, Type.Int), SLL(y, C(2)), (*これは4倍*)
            (Ans(FLd(x, V(offset))), Mloc.d_virtual_remain)) (*このansにmlocが足りなさそう*)
      | Type.Array(_) -> (*Ans(Ld(x, V(y), 4)) SPARCの場合*)
          Let((offset, Type.Int), SLL(y, C(2)), (*これは4倍*)
  ...
  | Closure.Put(x, y, z) ->
      ...
      (match M.find x env with
      | Type.Array(Type.Unit) -> Ans(Nop)
      | Type.Array(Type.Float) -> (* Ans(FSt(z, x, V(y), 8)) SPARC版*)
          Let((offset, Type.Int), SLL(y, C(2)),
            (Ans(FSt(z, x, V(offset))), Mloc.d_virtual_remain))
      | Type.Array(_) -> (*Ans(St(z, x, V(y), 4)) SPARC版*)
          Let((offset, Type.Int), SLL(y, C(2)),
              (Ans(St(z, x, V(offset))), Mloc.d_virtual_remain))
      | _ -> assert false))
...
let f (Closure.Prog(fundefs, e)) =
  data := (let lzero = Id.L(Id.genid "lzero") in [(lzero,0.0)]);
